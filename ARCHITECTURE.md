...
## Schema 8 / Mobile Schema 5：存钱计划

新增 savings_plans 与 savings_plan_accounts。计划保存目标、模式、手动进度、目标日期、状态与软删除字段；关联表支持多账户。自动进度实时汇总账户余额，不保存派生金额。退款仍以关联 REFUND 交易保存，列表查询通过原交易ID聚合展示。账户余额修正写入 ADJUSTMENT，不直接改期初余额。移动备份格式升级至schema 5并兼容读取schema 4。

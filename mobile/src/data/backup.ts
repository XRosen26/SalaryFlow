...
export async function buildBackup(db: SQLiteDatabase): Promise<BackupDocument> {
  ...
  return { ...base, integrity: { algorithm: 'SHA-256', digest: await digest(JSON.stringify(base)) } };
}
...
export async function writeBackup(db: SQLiteDatabase, recovery = false) {
  ...
  const directory = recovery ? new Directory(Paths.document, 'SalaryFlow', 'recovery') : Paths.cache;
  if (recovery && !directory.exists) directory.create({ intermediates: true, idempotent: true });
  const file = new File(directory, backupName(recovery ? 'Recovery' : 'SalaryFlow-Android'));
...
async function parseBackup(text: string): Promise<BackupDocument> {
  if (text.length > MAX_BACKUP_BYTES) throw new Error('备份文件超过 50 MB 限制');
  let value: unknown;
  try {
  ...
  if (value.format !== 'salaryflow-ledger' || value.formatVersion !== 1 || value.schemaVersion !== 4) {
    throw new Error('备份格式或版本不受支持');
  }
  assertRecord(value.tables, '备份缺少数据表');
  assertRecord(value.integrity, '备份缺少完整性校验');
  for (const table of TABLES) {
    if (!Array.isArray(value.tables[table])) throw new Error(`备份缺少 ${table} 数据`);
  }
  const document = value as unknown as BackupDocument;
  const { integrity, ...base } = document;
  if (integrity.algorithm !== 'SHA-256' || integrity.digest !== await digest(JSON.stringify(base))) {
    throw new Error('备份完整性校验失败，文件可能已损坏或被修改');
  }
  return document;
}

export async function inspectBackup(uri: string): Promise<{ document: BackupDocument; summary: BackupSummary }> {
  const file = new File(uri);
  if (!file.exists) throw new Error('无法读取所选文件');
  const info = file.info();
  if (typeof info.size === 'number' && info.size > MAX_BACKUP_BYTES) throw new Error('备份文件超过 50 MB 限制');
  const document = await parseBackup(await file.text());
  return {
    document,
    summary: {
      createdAt: document.createdAt,
      accounts: document.tables.accounts.filter((row) => row.deleted !== 1).length,
      transactions: document.tables.transactions.filter((row) => row.deleted !== 1).length,
      categories: document.tables.categories.filter((row) => row.archived !== 1).length,
    },
  };
}
...
export async function restoreBackup(db: SQLiteDatabase, document: BackupDocument) {
  ...
  await db.withExclusiveTransactionAsync(async (tx) => {
    ...
    const foreignKeyErrors = await tx.getAllAsync<Record<string, unknown>>('PRAGMA foreign_key_check');
    if (foreignKeyErrors.length) throw new Error('备份存在无效的数据关联，恢复已回滚');
    const transactionIntegrity = await tx.getFirstAsync<{ integrity_check: string }>('PRAGMA integrity_check');
    if (!transactionIntegrity || transactionIntegrity.integrity_check !== 'ok') throw new Error('数据库完整性检查失败，恢复已回滚');
  });

  const integrity = await db.getFirstAsync<{ integrity_check: string }>('PRAGMA integrity_check');
  if (!integrity || integrity.integrity_check !== 'ok') throw new Error('恢复后的数据库完整性检查未通过');
}

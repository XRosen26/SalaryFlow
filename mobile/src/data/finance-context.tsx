...
type FinanceContextValue = {
  ...
  setAccountSummaryVisible: (visible: boolean) => Promise<void>;
...
export function FinanceProvider({ children }: PropsWithChildren) {
  ...
  const value = useMemo<FinanceContextValue>(
    () => ({
      ...
      setAccountSummaryVisible: async (visible) => {
        await updateSettings(db, { accountSummaryVisible: visible });
        await refresh();
      },
      ...
      refundTransaction: async (originalId, amountMinor, date, note) => {
        const transactionId = await refundExpense(
          db,
          originalId,
          amountMinor,
          date,

...
import { createReceivable, deleteReceivable, loadReceivables, repayReceivable, type ReceivableItem } from "@/data/repository";
import { addDays, today } from "@/domain/dates";
import { formatMoney, parseMoneyExpression } from "@/domain/money";

function dueVisual(dueDate:string|null,status:string) {
  if(status==="SETTLED"||!dueDate)return {label:status==="SETTLED"?"已结清":"未设归还日",border:"transparent",background:"transparent",text:null as string|null,bold:false};
  const now=today();
  if(dueDate<now)return {label:"已逾期",border:"#991B1B",background:"#991B1B18",text:"#991B1B",bold:true};
  if(dueDate===now)return {label:"今日到期",border:"#F04438",background:"#F0443820",text:"#C4320A",bold:true};
  if(dueDate===addDays(now,1))return {label:"明日到期",border:"#F79009",background:"#F790091A",text:"#B54708",bold:false};
  if(dueDate===addDays(now,2))return {label:"2天后到期",border:"#EAAA08",background:"#EAAA0818",text:"#946200",bold:false};
  if(dueDate===addDays(now,3))return {label:"3天后到期",border:"#84AD0A",background:"#84AD0A18",text:"#668000",bold:false};
  return {label:"待归还",border:"#32A071",background:"#32A07110",text:"#217A55",bold:false};
}

export default function ReceivablesScreen() {
  const colors=useAppTheme(),db=useSQLiteContext(),params=useLocalSearchParams<{create?:string}>();
  const {snapshot,error,refresh}=useFinance();
  const [rows,setRows]=useState<ReceivableItem[]>([]),[creating,setCreating]=useState(params.create==="1"),[busy,setBusy]=useState(false);
  ...
  const remove=(row:ReceivableItem)=>Alert.alert("撤销待收款","仅在未实际借出或误操作时使用。撤销会同时移除借出和全部归还流水，并恢复相关账户余额。",[{text:"取消",style:"cancel"},{text:"确认撤销",style:"destructive",onPress:async()=>{try{setBusy(true);await deleteReceivable(db,{id:row.id,revision:row.revision,reason:"用户确认未实际借出或误操作"});await Promise.all([reload(),refresh()])}catch(e){Alert.alert("无法撤销",e instanceof Error?e.message:"请重试")}finally{setBusy(false)}}}]);
  ...
  return <AppScreen>
    ...
    <Card style={styles.list}>{rows.length?rows.map((row,index)=>{
      const due=dueVisual(row.dueDate,row.status);
      return <View key={row.id}>{index?<Divider/>:null}
        <View style={[styles.row,{backgroundColor:due.background,borderLeftColor:due.border,borderLeftWidth:due.border==="transparent"?0:4}]}>
          <View style={{flex:1,gap:5}}>
            <View style={styles.rowTop}><Text style={[styles.person,{color:colors.text,fontWeight:due.bold?"900":"800"}]}>{row.person}</Text>
              <Text style={[styles.status,{color:due.text??(row.status==="SETTLED"?colors.income:colors.primary),fontWeight:due.bold?"900":"800"}]}>{due.label}</Text></View>
            <Text style={[styles.meta,{color:colors.textSecondary}]}>{row.lentDate} 借出 · {row.sourceAccountName}{row.dueDate?" · 预计 "+row.dueDate:" · 未设归还日"}</Text>
            {row.note?<Text style={[styles.meta,{color:colors.textSecondary}]}>{row.note}</Text>:null}
          </View>
          <View style={styles.money}><MoneyAmount value={row.outstandingMinor} hidden={hidden} size={17}/>
            <View style={styles.itemActions}>{row.status==="OPEN"?<Pressable onPress={()=>openRepay(row)}><Text style={{color:colors.primary,fontWeight:"800"}}>登记归还</Text></Pressable>:null}
              <Pressable disabled={busy} onPress={()=>remove(row)} accessibilityLabel={`撤销 ${row.person} 的待收款`}><Ionicons name="trash-outline" size={19} color={colors.expense}/></Pressable></View>
          </View>
        </View>
      </View>})
      :<View style={styles.empty}><Ionicons name="people-outline" size={32} color={colors.primary}/><Text style={[styles.person,{color:colors.text}]}>暂时没有待收款</Text><Text style={[styles.meta,{color:colors.textSecondary}]}>借款可不设置归还日；设置后会在这里显示到期和逾期提醒。</Text></View>}
    </Card>    <Text style={[styles.foot,{color:colors.textSecondary}]}>借出与归还是资产形态变化，不计入收入、支出、预算执行率或储蓄率。</Text>
  </AppScreen>;
}

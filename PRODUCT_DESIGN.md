...
## 0.9安心支出与历史周期选择（2026-09-16）

总览卡把计划额度和真实资金作为并行约束：主金额仍展示剩余预算，次级区域并列主要消费账户余额和可支出金额。卡片文案、颜色和说明按两项约束中更紧张的一项变化；无实际资金时使用明确红色资金不足状态，不以较高预算比例掩盖问题。

PC和移动统计均采用“常用范围 + 日历周期快捷选择 + 自定义日期”三层结构。日历周期先选周/月/年，再打开倒序周期列表；列表只追溯到首次记账所在周期，条目同时展示名称和完整起止日期。移动端用可滚动弹层避免小屏塞入长列表，自定义日期使用统一日历/文本日期组件。

## 0.8.1待收款贯通与到期提醒（2026-09-14）

交易明细的时间与金额范围是独立筛选维度，可与类型、账户、分类、搜索和排序组合。统计增加今日与最近3天，仍按实际发生日期计算。

“待收款”作为独立资产流程，而不是支出分类：借出和归还只改变账户资金与待收余额，不进入收支、预算或储蓄率。对方必填，预计归还日可选；设置日期后显示到期/逾期提醒，允许分次回款和每次选择实际账户。

移动记账保持“先定语义再填金额”，但分类和账户改为短入口与底部弹层，选中即收起。预算用完或超支后首页卡片改为状态提醒。Android/iOS共用`mobile/`业务层与触屏交互，iOS原生包另在macOS上验证。

## 0.7.1桌面构成分析与Android 0.3视觉完善（2026-09-13）

桌面构成分析增加“收入与净支出”，帮助比较所选时间内两类资金流的体量；比例不是储蓄率，界面保持独立储蓄率指标。圆环主色、辅助色、直接标签和悬停强调跟随六套界面配色及浅/深主题，避免所有图形退化为同一种浅灰。

Android设置页提供与桌面一致的六套配色。预算列表仍按七级预算执行率显示单项风险；首页总览卡同时参考本期剩余预算和主要消费账户余额，以更紧张的一项决定颜色与提示，避免有预算但没有实际资金时仍显示绿色。快速记账以分组、图标和文字完整展示动态分类，分类较多时允许纵向滚动，并保留分类管理入口。

Android安装名保持短而可识别：简体中文系统显示“薪流”，默认及英文系统显示“SalaryFlow”。应用图标采用稳定S形资金流标志；预算风险不改变启动器图标，避免图标频繁变化、缓存不同步和在桌面泄露财务状态。
...
帮助页增加独立“关于作者”，仅展示用户确认的简洁信息：“由 XRosen26 完成并持续迭代。”不从其他对话推断或拼接个人资料。
...
版本：V1 已确认设计基线｜更新至 Windows 0.9.0 / Android 0.6.0。本文同时保留后续交互目标，实际范围及差异见 [IMPLEMENTATION.md](./IMPLEMENTATION.md)。
...
| 固定账单       | 到期待办、处理历史、周期规则                                 | 确认支付、关联支出、延期、跳过、修改与删除规则   | 待办→交易；规则→未来待办；历史→原交易        |
| 工资分配       | 工资选择、分配依据、计划、确认与历史                         | 按预算缺口生成计划；确认真实转账；取消/删除计划  | 工资收入→分配；真实转账→交易                 |
| 待收款         | 待收列表、临近提醒、借出、分次归还、撤销                     | 管理临时借出资产，不计收支与预算                 | 关联资金流水→交易；归还→指定账户             |
| 账户           | 账户列表、账户详情、账户编辑、余额校准、角色设置             | 动态账户、类型、默认角色、资产与余额             | 明细→交易；校准→调整记录；归档→引用影响      |
| 存钱计划       | 目标列表、关联账户进度、手动进度、目标日期                   | 目标、目前、还差、完成比例；修改与删除           | 账户余额→进度；只读关联，不移动资金          |
| 分析           | 收支趋势、分类分布、账户筛选、区间比较、明细表               | 多时间范围、账户维度、构成、排序和导出           | 图形→交易；工资周期→预算；按实际日期统计     |
| 帮助与使用手册 | 功能说明、财务口径、界面示例、关于                           | 快速入门、解释禁用原因和数据边界                 | 侧栏直达；与设置内帮助页签一致               |
| 设置与数据     | 偏好、分类、固定账单快捷入口、导入导出、备份恢复、外观、关于 | 低频配置、数据可迁移性和修复                     | 导入→暂存预览/结果；备份→恢复向导            |

当前Windows侧栏提供总览、交易记录、预算与周期、固定账单、工资分配、待收款、我的账户、存钱计划、统计分析、帮助与使用手册、设置与数据十一个明确入口。固定账单管理也可从设置快捷进入同一页面，到期待办在总览和固定账单页均可处理。
...
存钱计划（已实现）
应急金  8,000/20,000  关联：长期储蓄    截止日... [修改]
旅行    1,000/5,000   手动进度             [调整]
关联模式只读取所选账户当前余额，不移动资金或创建交易；手动模式不修改账户余额
```
...
存钱计划独立于预算：预算回答“本期可以花多少”，存钱计划回答“目标还差多少”。关联账户模式适合应急金、旅行金等专用账户；手动模式适合资金分散或不希望纳入账户余额的目标。移动端余额修改使用确认式校准，不直接覆盖历史流水。超支补齐只调整真实账户资金，界面同时提醒用户重新评估预算或减少非必要开支。
